...
function StudioPrompts() {
  ...
  return <><Header eyebrow="AI STUDIO" title="Prompt Registry" description="Create reusable prompts, manage immutable versions, then attach them to runtime stages in Conversation Flow." actions={<><div className={cx("runtime-pill", runtime.data?.registryConnected && "connected")}><span />{runtime.data?.registryConnected ? `Runtime connected · ${runtime.data.release?.release_code}` : "Runtime configuration required"}</div><button className="button primary" onClick={() => setEditing(null)}><Plus size={16} />New prompt</button></>} />
    {actionError && <div className="error-box page-error"><AlertTriangle size={17} />{actionError}</div>}
...
function Releases() {
  ...
  return <><Header eyebrow="AI STUDIO" title="Release Control" description="Pin published prompts and model settings, require approval, pass the regression gate, then activate the immutable runtime bundle." actions={<button className="button primary" onClick={() => setCreating(true)}><Plus size={16} />Create candidate</button>} />
    {actionError && <div className="error-box page-error"><AlertTriangle size={17} />{actionError}</div>}
